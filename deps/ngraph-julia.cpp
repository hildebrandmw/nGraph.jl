...
JLCXX_MODULE define_julia_module(jlcxx::Module& mod)
{
    ...
    mod.method("makeshape", [](const jlcxx::ArrayRef<size_t, 1> vals){
        std::vector<size_t> v = {};
        for (auto i: vals) {
            v.push_back(i);
        }
        return ngraph::Shape(v);
    });
   ...
   // Onnx models
    ...
    mod.method("create_tensor", [](ngraph::runtime::Backend* backend, const ngraph::element::Type& element_type,  const jlcxx::ArrayRef<size_t, 1> size){
            // Create a Shape type
            std::vector<size_t> v = {};
            for (auto i: size) {
                v.push_back(i);
            }
            auto shape = ngraph::Shape(v);
